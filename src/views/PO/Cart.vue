...
export default {
  ...
  methods: {
    ...
    fetchData() {
      service
        .fetchData('/api/po/cart?user=' + this.userid)
        .then((response) => {
          this.totalHarga = 0
    ...
    checkOutCart() {
      ...
      if (validasi_checkout === true) {
        ...
        service
          .postData(
            '/api/po/checkout?user=' + this.userid,
            this.checkout,
          )
          .then((response) => {
            if (response.status === 'ok') {
    ...
    addCartItem() {
      ...
      if (validasi_cart === true) {
        this.isLoading = true
        service
          .postData('/api/po/cart?user=' + this.userid, this.cartitem)
          .then((result) => {
            setTimeout(() => {
              this.isLoading = false
            }, 1000)
    ...
    getSupplier() {
      service
        .fetchData('/api/supplier?all=true')
        .then((response) => {
