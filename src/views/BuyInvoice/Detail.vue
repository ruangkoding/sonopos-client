                ...
                <table class="table table-bordered">
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td style="width:85%;">
                        {{ invoice.info.supplier.supplier_name }}<br />
                        {{ invoice.info.supplier.supplier_address }}<br />
                        {{ invoice.info.supplier.supplier_contact }}
                      </td>
                    </tr>
                    <tr>
...
export default {
  data() {
    return {
      ...
      payment: {
        buy_invoice_id: this.$route.params.id,
        note: '',
        payment_date: '',
        total: '',
        nominal: '',
  ...
  methods: {
    getInvoice() {
      service
        .fetchData(this.$apiUrl + '/order/buy/show/' + this.$route.params.id)
        .then((response) => {
    ...
    processPayment() {
      ...
      if (validasi === true) {
        if (this.payment.nominal <= this.payment.total) {
          service
            .postData(this.$apiUrl + '/order/buy/pay', this.payment)
            .then((response) => {
    ...
    checkRemainingPayment() {
      service
        .fetchData(this.$apiUrl + '/order/buy/pay?invoice=' + this.invoice.id)
        .then((response) => {
