    ...
    <div class="card-body">
      ...
      <transition name="fade">
        <table class="table table-bordered">
          <tbody>
            <tr>
              <td style="width:15%;"><b>Nomor Order</b></td>
              <td style="width:85%;">{{ so.so_number }}</td>
            </tr>
            <tr>
              <td style="width:15%;"><b>Tanggal Order</b></td>
...
export default {
  ...
  methods: {
    getSO() {
      service
        .fetchData('/api/so/show/' + this.$route.params.id)
        .then((response) => {
          this.so = response
          this.sodetail = this.so.detail
          this.checkout.total = this.so.total
    ...
    generateInvoice() {
      ...
      if (validasi === true) {
        service
          .postData(
            'api/so/generateinvoice?id=' + this.po.id,
            this.checkout,
          )
          .then((response) => {
            if (response.status === 'ok') {
              ...
              window.location.href =
                '/api/so/poinvoice/detail?id=' + response.invoice_id
            }
          })
          .catch((error) => {
            this.$swal(
    ...
    rejectOrder() {
      service
        .postData('api/so/rejectso?id=' + this.so.id)
        .then((response) => {
          if (response.status === 'ok') {
    ...
    rejectItem() {
      service
        .postData('/api/so/rejectitem?item=' + this.itemId)
        .then((response) => {
          if (response.status === 'ok') {
